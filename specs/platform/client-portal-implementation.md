...
Loads the latest snapshot (weekly → monthly → quarterly fallback), renders, in order: the score hero (score/grade/delta/one-line takeaway); 5 supporting cards (search clicks, AI hit-rate summary, reviews, "what we did for you," and domain rank — see below); then the 5-pillar breakdown (each bar linking to its pillar page, colored by that pillar's own score band — never by rank among the 5). Friendly empty state if no snapshot exists at all. Copy for all of this lives in one module, `geo_agent/portal_copy.py` (`PILLAR_COPY`, `FOUNDATION_ITEM_COPY`, `grade_takeaway()`, plus a `PILLAR_KEY_TO_SLUG`/`PILLAR_SLUG_TO_KEY` mapping between the DB's stable pillar keys and the portal's URL slugs) — not yet reviewed by marketing.

*Note on scope:* Ticket 4's original acceptance criteria explicitly excluded domain-authority/competitor-gap detail from this page ("the customer is a dental practice owner, not a marketer"). That decision was later reversed by direct request — the 5th card ("Domain rank") shows Domain Authority, trend vs. last month, and the top competitor's DA, reusing `sections.authority` from the same frozen snapshot every other card reads (gated on DA being tracked for that customer at all; renders a friendly empty state otherwise). Card order and the section reorder (cards now above the pillar breakdown, not below) were both later direct requests too, not part of the original ticket set.
